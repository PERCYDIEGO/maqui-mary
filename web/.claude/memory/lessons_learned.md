...
- **Seguimiento (commit `50c6141`)**: al probar "Anular" en sandbox, salió el mensaje genérico "Error al anular el documento" (sin detalle) — mismo patrón de bug ya visto antes con el envío normal (`apiResult.message` puede venir vacío en ciertos casos de error de APISUNAT.pe). Se agregó fallback a `JSON.stringify(apiResult.payload)` y un `console.error` con el request+response completos para poder diagnosticar la próxima vez que ocurra, ya que no hay forma de ver logs históricos de Vercel desde este entorno (solo streaming en vivo). **Pendiente**: Percy va a reintentar y pasar el mensaje de error detallado que salga ahora.
- **Resultados observados en las pruebas de sandbox (misma sesión)**: (1) F001-884 → "La Factura F001-884 ya ha sido anulada" — confirma que el request SÍ llega bien formado a APISUNAT.pe (si no, daría error de validación, no este mensaje). Verificado que nuestra BD no se tocó (aislamiento sandbox funcionando). (2) F001-883 → "APISUNAT.pe no devolvió detalle del error" — caso vacío sin `message` ni `payload`. Se intentó capturar el log en vivo con `vercel logs` en 2 ventanas de 45-60s sin éxito (problema de sincronización de timing entre pedir el reintento y empezar a escuchar). **Solución adoptada (commit `91638a3`)**: en vez de depender de logs del servidor, se agregó `debug_raw` a la respuesta de error — el JSON completo (`{enviado, respuesta}`) ahora se muestra directo en el toast del navegador, así el usuario puede pegarlo sin que se necesite acceso a logs de Vercel en tiempo real.
- **LECCIÓN DE PROCESO**: `vercel logs <dominio>` en streaming solo captura eventos DESPUÉS de que el comando arranca — no hay forma confiable de "pedirle al usuario que reintente" y coincidir el timing exacto. Para depurar errores de APIs externas en producción sin acceso a logs históricos, la estrategia correcta es devolver el detalle crudo de la respuesta directo en el payload de error que ve el usuario (como ya se hizo para el envío normal con `result.mensaje`), no intentar cazar logs en vivo.

---

### [2026-07-01] Estado "aprobado" no se corregía a "anulado" cuando SUNAT confirmaba después (sin webhook/polling)

- **CONTEXTO**: Percy anuló F001-883 y F001-884 en producción. La respuesta de APISUNAT.pe vino `PENDIENTE` (no `ACEPTADO` inmediato — las facturas también pueden ser asíncronas en `/api/v3/voided`, no solo boletas). Nuestro código correctamente NO marcó `anulado` hasta que SUNAT confirmara, pero **no había ningún mecanismo para enterarse de esa confirmación después** — el documento se quedaba mostrando "Aprobado" para siempre.
- **CORRECCIÓN** (commit `ccccfb3`): si un reintento de `/api/sunat/void` recibe el mensaje `"ya ha sido anulada"` / `"ya fue anulada"` de APISUNAT.pe, se interpreta como confirmación real (no como error) y se actualiza `estado_sunat='ANULADO'` + `data_json.estado='anulado'`. A diferencia del bug de "ya fue emitido" (que asumía `ACEPTADO` sin evidencia real y causó que una guía rechazada se mostrara como aprobada), acá el mensaje **sí es una señal inequívoca** — "ya ha sido anulada" no tiene otra interpretación posible, por eso es seguro actuar sobre ella.
- **REGLA**: Cuando un proceso externo es asíncrono y no hay webhook/polling para enterarse del resultado final, diseñar el flujo para que un reintento manual del usuario sirva como mecanismo de "reconciliación" — detectar mensajes de "ya procesado" y usarlos para corregir el estado atrasado, en vez de tratarlos siempre como error. Diferenciar caso por caso si el mensaje de "ya procesado" es una confirmación inequívoca (anular → siempre significa anulado) o ambigua (emitir → "ya emitido" no dice si fue aceptado o rechazado) antes de decidir si actuar automáticamente sobre él.

---

### [2026-07-01] Historial de Envíos ordenado por fecha de creación del borrador en vez de fecha real de envío

- **CONTEXTO**: Percy pidió que el historial siempre muestre lo más reciente primero, y un contador de documentos enviados.
- **CAUSA**: `documentosEnviados` en `sunat/page.tsx` se ordenaba con `sortDesc` (por `createdAt`, fecha de creación del borrador) — un documento creado hace días pero recién enviado hoy no aparecía arriba. Además, el filtro de esa lista no incluía `estado === 'anulado'`, así que un documento anulado desaparecía completamente del historial en vez de quedar registrado ahí. El contador ya existía (badge con `documentosEnviados.length`) pero no tenía texto explícito.
- **CORRECCIÓN** (commit `48c3dc1`): nuevo `sortPorEnvio` que ordena por `enviadoAt` (con fallback a `createdAt` si no existe) descendente. Se agregó `'anulado'` al filtro de boletas/facturas en `documentosEnviados`. Nuevo caso `'anulado'` en `getEstadoBadge` (antes caía al `default` sin estilo propio). Contador con texto explícito "X documentos enviados".
- **REGLA**: Al ordenar una lista de "eventos" (envíos, aprobaciones, etc.), usar la fecha del EVENTO relevante (`enviadoAt`), no la fecha de creación del registro subyacente (`createdAt`) — son conceptualmente distintos y pueden divergir mucho en el tiempo.
