...
- **CORRECCIÓN**: Implementada en commit `6f9bea3`:
  1. `AppContext.tsx`: `typeof dataJson.X === 'string' ? dataJson.X : null` antes de usar campos texto; `String()` en todos los fallbacks de columnas individuales.
  2. `documentos/page.tsx`, `guias/page.tsx`, `guias/nueva/page.tsx`: `String(valor || '').toLowerCase()` en todos los filtros.
...
- **CORRECCIÓN**: Implementada en commit `6f9bea3` — `String(g.destinatarioNombre || '')` y `String(g.numeroCompleto || '')` en todos los filtros de guías. También `guias/nueva/page.tsx` línea 63 donde `.toLowerCase()` se llamaba directamente sobre el campo sin null-guard ni String().
